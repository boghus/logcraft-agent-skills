...
Macro and micro context can coexist within the same operation.
...
When a result is already known, include it when it is relevant:
...
Do not predict future consequences in the current log. A log describes observable facts and known state at the time of the event.
